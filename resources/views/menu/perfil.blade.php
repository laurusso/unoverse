...
{{ csrf_field() }}
    ...
    <div class="perfil">
        ...
        <div class="quadro-perfil-esquerda">
            ...
            <a class="btnFoto-perfil" id="esconderBTN"  onclick="exibicaobutton('esconde')">cancelar</a>
                <button class="btnFoto-perfil"  id="Fotodefault">Excluir foto</button>
       ...
       <select  id="select" class="input-editavel"  name="id_escola">
        <!--Criar um foreach com opções do banco-->
        <!-- <option value="" data-default disabled selected></option>  -->
        @foreach($escolas as $esc)
            @if($esc->codigo_escola == $id_escola)
                <option value="{{$esc->codigo_escola}}" selected>{{$esc->nome_escola}}</option>
            @else
    ...
    @foreach($escolas as $esc)
            @if($esc->codigo_escola == $id_escola)
    <input class="input-editavel" type="text" id="editableInputEscola" value="{{$esc->nome_escola}}" readonly>
    @endif
        @endforeach 
             
                <i id="editIconEscola" class="icon-edit bi bi-pencil-square" onclick="toggleEditMode('editableInputEscola', 'editIconEscola', 'checkIconEscola','escola')"></i>
                <i id="checkIconEscola" class="icon-check bi bi-check" onclick="toggleEditMode('editableInputEscola', 'editIconEscola', 'checkIconEscola','escola')"></i>
